...
###Number of stocks in pf5 and pf1
n_stocks_pf <-  matrix(NA, nrow=5, ncol=ndates)
...
lene2 <- t(monthly_return)
colnames(lene2) <- c("pf1", "pf2","pf3", "pf4", "pf5")


lene3 <- lene2 %>% as_tibble
lene3$month <- rownames(lene2)
...
lene5 <- lene4 %>% mutate(year_month = substring(month, 1, 7))




return <- ggplot(lene5, aes(x = year_month, y = penge), col=col) + 
  #geom_point(aes(color=lene5$col)) +
  ...
  scale_x_discrete(breaks = unique(lene5$year_month)[c(T, rep(F,5))])




png(filename="C:/Users/leneb/Desktop/Speciale/Data/New_pictures/return_standardE_equal_fullsample.png",
...
lullu <- t(n_stocks_pf)
colnames(lullu) <- c("pf1", "pf2","pf3", "pf4", "pf5")


lullu1 <- lullu %>% as_tibble
lullu1$month <- rownames(t(monthly_return))
...
lullu3 <- lullu2 %>% mutate(year_month = substring(month, 1, 7))




n_stocks <- ggplot(lullu3, aes(x = year_month, y = penge), col=col) + 
  geom_point(aes(color=col)) +
  geom_line(aes(group=col, color = col)) + 
  scale_color_brewer(palette = "RdYlGn")+
  scale_fill_brewer(palette = "RdYlGn") + 
  xlab("Month") + ylab("Number of stock, value weighted full sample pf") +
  theme_classic()+
  theme(legend.title = element_blank()) +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10))+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_x_discrete(breaks = unique(lullu3$year_month)[c(T, rep(F,5))])




png(filename="C:/Users/leneb/Desktop/Speciale/Data/New_pictures/n_stock_pf_standardE_value_fullsample.png",
    res=500, units="in", width = 7.5, height = 3.5)
    n_stocks
dev.off()
