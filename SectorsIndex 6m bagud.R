...
cap_sector <- matrix(0, nrow =ngroups, ncol = 11)
...
for (i in 1:(ndates)){
  ...
  for (l in 1:11){
    ...
    joined_temp_s <- joined_temp_s %>%  #Creates a new variabel: cummulative sum 
      as_tibble() %>% 
      dplyr::mutate(Scap=ave(Cap, FUN=cumsum))
    
    frac <- (sum(joined_temp_s$Cap)/ngroups) #fraction for the next calculation
    q <- c(0, frac, frac*2, frac*3, frac*4, frac*5) #this is for 5 groups
    
    #I create a vector for the return of each pf
    #I create a vector for the market cap of each pf
    return_i <- vector()
    cap_i <- vector()
    ...
    for (j in 1:ngroups){
      ...
      pf <- joined_temp_s %>%
        dplyr::filter(between(joined_temp_s$Scap, q[j], q[j+1])) #approx same market cap in each
      
      #Calculate the return
      ###########THE ONLY PLACE TO CHANGE FOR EQUAL WEIGHTED RETURNS
    ...
    return_sector[,l] <- return_i 
    cap_sector[,l] <-cap_i
    
    }
  weighted_return <- vector()
  
  for (k in 1:ngroups){
    weighted_return[k] <-weighted.mean(return_sector[k,],cap_sector[k,])
  }
  
  equal_avE_sector_timseries[,i] <-equal_avE_sector
  weighted_avE_sector_timseries[,i] <-weighted_avE_sector
  
  monthly_return[,i] <- weighted_return
