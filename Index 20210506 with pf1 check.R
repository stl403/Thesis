...
share_large_stocks_pf_5 <- matrix(0, nrow=4, ncol=ndates)
large_stocks <- c('MSFT UW Equity',
                  'AAPL UW Equity', 'GOOG UW Equity', 'GOOGL UW Equity')

#CVX UN Equity

rownames(share_large_stocks_pf_5) <- large_stocks
colnames(share_large_stocks_pf_5) <- c(as.character(dates_0))
...
for (i in 1:(ndates)){
  ...
  #amount_i <- vector()
    ...
    for (j in 1:ngroups){
        ...
        return_i[j] <- mean(pf$Return)
          #sum(pf$Return*pf$Cap)/sum(pf$Cap) #OBS:Market cap -1 er taget med ind ved konstruktion af joined_temp, som pf er lavet af
        ...
        if(j==1){
        for (l in 1:4){
          if(large_stocks[l] %in% pf$Ticker){
            cap <-pf$Cap[pf$Ticker==large_stocks[l]]
            share_large_stocks_pf_5[l,i] <- cap/sum(pf$Cap)
          }
        }
        }
    }
...
###########################################
share_large_stocks_pf_5_percent <- share_large_stocks_pf_5*100

Google <- share_large_stocks_pf_5_percent[3,]+share_large_stocks_pf_5_percent[4,]
sum <- vector()

for (k in 1:ndates){
  sum[k] <- sum(share_large_stocks_pf_5_percent[,k])
}


share_large_stocks_finnish <- cbind(t(share_large_stocks_pf_5_percent[1:2,]), Google, sum)

lone <- share_large_stocks_finnish
colnames(lone) <- c("Microsoft", "Apple", "Google", "Sum")


lone2 <- lone %>% as_tibble
lone2$month <- rownames(lone)

lone3 <- lone2 %>%
  as_tibble %>%
  gather(key = "col", value = "Share", -month)

lone4 <- lone3 %>% mutate(year_month = substring(month, 1, 7))


percentage_share <- ggplot(lone4, aes(x = year_month, y = Share), col=col) + 
  geom_point(aes(color=col)) +
  geom_line(aes(group=col, color =col)) + 
  xlab("Month") + ylab("Percentage share of portfolio 5") +
  theme_classic()+
  theme(legend.title = element_blank())+
  scale_y_continuous(breaks = scales::pretty_breaks(n = 20))+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  scale_x_discrete(breaks = unique(lone4$year_month)[c(T, rep(F,5))])


png(filename="C:/Users/leneb/Desktop/Speciale/Data/New_pictures/percentagesharePF5.png",
